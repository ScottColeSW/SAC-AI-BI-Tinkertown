...
def create_load_data(num_customers, time_step, flights_daily):
    ...
    #     'airline': np.random.choice([1:'UA', 2:'AA', 3:'DL'], num_flights),
    #     'origin': np.random.choice([1:'SFO', 2:'LAX', 3:'ORD', 4:'JFK'], num_flights),
    #     'destination': np.random.choice([1:'SFO', 2:'LAX', 3:'ORD', 4:'JFK'], num_flights),
    #     'departure_time': np.random.randint(0, 24, num_flights),
    #     'flight_path': [np.random.choice([1:'A', 2:'B', 3:'C'], 10) for _ in range(num_flights)],
    #     'runway': np.random.choice([1, 2, 3, 4], num_flights),
    #     'landing_time': np.random.randint(0, 24, num_flights)
    # })

    # flight_metadata = pd.DataFrame({
    #     'airline_codes': [1:'UA', 2:'AA', 3:'DL'],
    #     'origin_codes': [1:'SFO', 2:'LAX', 3:'ORD', 4:'JFK'],
    #     'destination_codes': [1:'SFO', 2:'LAX', 3:'ORD', 4:'JFK'],
    #     'flight_paths': [1:'A', 2:'B', 3:'C']
    #     })

    flights = pd.DataFrame({
        'airline': np.random.choice([1, 2, 3], num_flights),
        'origin': np.random.choice([1, 2, 3, 4], num_flights),
        'destination': np.random.choice([1, 2, 3, 4], num_flights),
        'departure_time': np.random.randint(0, 24, num_flights),
        'flight_path': [np.random.choice([1, 2, 3], 10) for _ in range(num_flights)],
        'runway': np.random.choice([1, 2, 3, 4], num_flights),
        'landing_time': np.random.randint(0, 24, num_flights)
    })

    # # Define a schema for the flight data
    # flight_schema = {
    #     'airline': 'int',
    #     'origin': 'int',
    #     'destination': 'int',
    ...
    # # Generate random values for each feature in the schema
    # flight_data = []
    # for i in range(num_flights):
    #     flight = {}
    #     for feature in flight_schema:
    #         if flight_schema[feature] == 'int': #was string
    #             # 'UA', 'AA', 'DL'
    #             flight[feature] = np.random.choice([1, 2, 3])
    #         elif flight_schema[feature] == 'int':
    #             flight[feature] = np.random.randint(0, 24)
    #         elif flight_schema[feature] == 'int': #was list
    #             # 'A', 'B', 'C'
    #             flight[feature] = np.random.choice([1, 2, 3])
    #     flight_data.append(flight)

    # # Convert the flight data to a NumPy array
    # flight_data_array = np.array(flight_data)
    # Convert the DataFrames to NumPy arrays
    customer_data_array = customer_data.to_numpy()
    bookings_array = bookings.to_numpy()
    flight_data_array = flights.to_numpy()

    # Convert the NumPy arrays to PyTorch tensors
    customer_data_tensor = torch.tensor(customer_data_array)
    bookings_tensor = torch.tensor(bookings_array)
    flights_tensor = torch.tensor(flight_data_array)

    # # Add meta context to the tensor
    # flight_data_meta_context = {
    # 'airline_names': ['UA', 'AA', 'DL'],
    # 'airport_codes': ['SFO', 'LAX', 'ORD', 'JFK']
    # }

    # # Combine the tensor and the meta context into a single object
    # flight_data = {
    # 'tensor': flights_tensor,
    # 'meta_context': flight_data_meta_context
    # }

    return customer_data_tensor, bookings_tensor, flights_tensor, customer_data, bookings, flight_data_array
...
def train_and_evaluate(AIModel, data_df, target_column, validation_split=0.25):
    ...
    #X_train, X_test, y_train, y_test = train_test_split(data_tensor, test_size=validation_split)
    #X_train, X_test, y_train, y_test = train_test_split(data_tensor, test_size=validation_split, random_state=42)
#    X_train, X_val, X_test, y_train, y_val, y_test = train_test_split(data_tensor, test_size=validation_split, train_size=0.75, random_state=42)

    # Split the data into training, validation, and test sets
    #X_train, X_val, X_test, y_train, y_val, y_test = train_test_split(data_tensor, test_size=validation_split, train_size=0.75, random_state=42)
    X_train, X_test, y_train, y_test = train_test_split(data_tensor, test_size=validation_split, train_size=0.75, random_state=42)
    #train_test_split(data_tensor, .3, .7, 42, True)
