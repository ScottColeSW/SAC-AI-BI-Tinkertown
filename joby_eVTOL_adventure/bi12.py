...
# Or loop through the columns
for col in data.columns:
    data[col] = pd.to_numeric(data[col])

# Convert airline to numeric
airline_dummies = pd.get_dummies(data['airline']) 

# Drop string columns  
data = data.drop(columns=['airline', 'flight_path'])

# Join the encoded columns
data = data.join(airline_dummies)

# Convert the `object` data type in the DataFrame to a numeric data type
...
torch.save(model.state_dict(), 'sac-sat-model.pt')
